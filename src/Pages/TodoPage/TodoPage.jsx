...
const TodoPage = () => {
  ...
  const addTodo = (todo) => {
    ...
    setTodos((prevTodos) => [
      ...prevTodos,
      { ...todo, id: Date.now(), dateAdded: moment().format("MMMM Do YYYY") },
    ]);
  };
  ...
  const setDueDate = (id, dueDate) => {
    ...
    setTodos((prevTodos) =>
      prevTodos.map((prevTodo) =>
        prevTodo.id === id ? { ...prevTodo, dueDate: dueDate } : prevTodo
      )
    );
  };
    ...
    >
      <div className="dark:bg-bgDark">
        <div className="w-[90%] mx-auto md:w-[80%] py-12 md:py-16 ">
          <h1 className="text-2xl md:text-4xl xl:text-6xl mb-6 font-semibold text-purpleMain text-center capitalize">
            welcome {username}!
          </h1>
          <div className="flex items-center justify-center my-2">
            <Button onClick={saveTodos} disabled={!hasInteracted}>
              {isSaving ? (
                <PulseLoader color="#7871fc" size={8} />
              ) : (
                "Save Todos"
              )}
            </Button>
