...
const Button = ({ isIndex = "primary", type="", width="", disabled=false, children, onClick, TodoFieldButton=false }) => {
  ...
  return (
    <button className={`${baseClasses} ${TodoFieldButton ? TodoFieldClasses : ""} ${isIndex === "primary" ? primaryClasses : secondaryClasses} ${spacing}`} type={type} disabled={disabled} onClick={onClick}>
      {children}
    </button>
  );
};
