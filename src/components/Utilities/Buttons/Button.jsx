...
const Button = ({ isIndex = "primary", type="", width="", disabled=false, children }) => {
  ...
  return (
    <button className={`${baseClasses} ${isIndex === "primary" ? primaryClasses : secondaryClasses} ${spacing}`} type={type} disabled={disabled}>
      {children}
    </button>
  );
};
